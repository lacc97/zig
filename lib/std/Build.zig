...
pub const Dependency = struct {
    ...
    pub fn path(d: *Dependency, sub_path: []const u8) LazyPath {
        return .{ .owner = d.builder, .path = sub_path, .root = .build };
    }
};
...
pub const GeneratedRoot = struct {
    /// The step that generates this directory
    step: *Step,

    /// The path to the generated root directory. Must be either absolute or relative to the build root.
    /// This value must be set in the `fn make()` of the `step` and must not be `null` after the step has ran.
    path: ?[]const u8 = null,

    pub fn getPath(self: GeneratedRoot) []const u8 {
        return self.path orelse std.debug.panic(
            "getPath() was called on a GeneratedRoot that wasn't built yet. Is there a missing Step dependency on step '{s}'?",
            .{self.step.name},
        );
    }
};
...
pub fn path(b: *Build, p: []const u8) LazyPath {
    ...
    return .{ .owner = b, .path = b.dupePath(p), .root = .build };
}
...
pub const LazyPath = struct {
    ...
    ///
    root: union(enum) {
        /// The path is evaluated relative to the owner build root.
        build,

        /// The path is evaluated relative to the current working directory of the build runner.
        /// Only for this tag is the path allowed to be absolute.
        ...
        generated: *const GeneratedRoot,

        /// A file that is generated by an interface or its parent directory. Those files usually are
        /// not available until built by a build step.
        generated_file: struct {
            file: *const GeneratedFile,

            /// The number of parent directories to go up.
            /// 0 means the generated file,
            /// 1 means the parent of the generated file, and so on.
            up: usize,
        },
    },
    ...
    pub fn generated(g: *const GeneratedRoot, p: []const u8) LazyPath {
        // TODO: handle absolute path
        return .{ .owner = g.step.owner, .path = g.step.owner.dupePath(p), .root = .{ .generated = g } };
    }

    pub fn generatedFile(g: *const GeneratedFile) LazyPath {
        return .{ .owner = g.step.owner, .path = "", .root = .{ .generated_file = .{ .file = g, .up = 0 } } };
    }
    ...
    pub fn dirname(self: LazyPath) LazyPath {
        return .{
            .owner = self.owner,
            .path = switch (self.root) {
                .generated_file => self.path, // TODO(lacc97): do this the right way by path manipulation
                else => dirnameAllowEmpty(self.path) orelse {
                    if (self.root == .cwd and fs.path.isAbsolute(self.path)) {
                        ...
                        dumpBadDirnameHelp(null, null,
                            \\dirname() attempted to traverse outside the LazyPath root.
                            \\This is not allowed.
                            \\
                        , .{}) catch {};
                        @panic("misconfigured build script");
            ...
            .root = switch (self.root) {
                .build, .cwd, .generated => self.root,
                // TODO(lacc97): do this the right way by path manipulation (`root` field should not change as a result of calling `dirname`)
                .generated_file => |gen| .{
                    .generated_file = .{ .file = gen.file, .up = gen.up + 1 },
                },
            },
        };
    }
    ...
    pub fn getDisplayName(self: LazyPath) []const u8 {
        return switch (self.root) {
            // TODO: different display string for what used to be `dependency` case?
            .build, .cwd => self.path,
            .generated, .generated_file => "generated",
        };
    }
    ...
    pub fn addStepDependencies(self: LazyPath, other_step: *Step) void {
        switch (self.root) {
            ...
            .generated_file => |gen| other_step.dependOn(gen.file.step),
        }
    }
    ...
    pub fn getPath2(self: LazyPath, src_builder: *Build, asking_step: ?*Step) []const u8 {
        const root_path = switch (self.root) {
            .build => blk: {
                if (self.owner != src_builder) assert(self.owner.build_root.path != null);
                break :blk self.owner.build_root.path orelse ".";
            },
            .cwd => blk: {
                const cwd = process.getCwdAlloc(src_builder.allocator) catch @panic("OOM");
                if (fs.path.isAbsolute(self.path)) {
                    // Special case where absolute path is allowed, so we handle
                    // it here instead of letting it out of the switch.
                    return fs.path.resolve(src_builder.allocator, &.{ cwd, self.path }) catch @panic("OOM");
                }
                break :blk cwd;
            },
            .generated => |gen| gen.path orelse {
            ...
            .generated_file => |gen| {
                ...
                const gen_step = gen.file.step;
                var p = gen.file.path orelse {
                    std.debug.getStderrMutex().lock();
                    const stderr = std.io.getStdErr();
                ...
                while (i < gen.up) : (i += 1) {
                    ...
                    if (mem.eql(u8, p, cache_root_path) and (i + 1) < gen.up) {
        ...
        // TODO: other failsafe checks (i.e. no funny business with ..)
        assert(!fs.path.isAbsolute(self.path));

        return fs.path.resolve(src_builder.allocator, &.{ root_path, self.path }) catch @panic("OOM");
    }
    ...
    pub fn dupe(self: LazyPath) LazyPath {
        return .{
            .owner = self.owner,
            // TODO(lacc97): do we really need to dupe path, cwd_relative? the LazyPath should already own its path
            .path = self.owner.dupePath(self.path),
            .root = switch (self.root) {
                .generated_file => |gen| .{
                    .generated_file = .{
                        .file = gen.file,
