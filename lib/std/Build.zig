...
pub fn pathGenerated(b: *Build, g: *const GeneratedFile) LazyPath {
    return .{ .owner = b, .path = "", .root = .{ .generated = g } };
}

// pub fn pathGeneratedRoot(b: *Build, g: *const GeneratedFile, p: []const u8) LazyPath {
//     return .{ .owner = b, .path = p, .root = .{ .generated_root = g } };
// }
...
pub const LazyPath = struct {
    ...
    root: union(enum) {
        ...
        generated_root: *const GeneratedFile,
    ...
    pub fn dirname(self: LazyPath) LazyPath {
        return .{
            ...
            .path = switch (self.root) {
                .generated, .generated_dirname => self.path, // TODO(lacc97): do this the right way by path manipulation
                .build, .cwd, .generated_root => dirnameAllowEmpty(self.path) orelse {
                    if (self.root == .cwd and fs.path.isAbsolute(self.path)) {
                        ...
                        dumpBadDirnameHelp(null, null,
                            \\dirname() attempted to traverse outside the LazyPath's root.
                            \\This is not allowed.
                            \\
                        , .{}) catch {};
                    }
    ...
    pub fn getDisplayName(self: LazyPath) []const u8 {
        return switch (self.root) {
            ...
            .build, .cwd => self.path,
            .generated, .generated_dirname, .generated_root => "generated",
        };
    }
    ...
    pub fn addStepDependencies(self: LazyPath, other_step: *Step) void {
        switch (self.root) {
            .build, .cwd => {},
            .generated, .generated_root => |gen| other_step.dependOn(gen.step),
    ...
    pub fn getPath2(self: LazyPath, src_builder: *Build, asking_step: ?*Step) []const u8 {
        switch (self.root) {
            .build => {
                ...
                return self.owner.pathFromRoot(self.path);
            },
            .cwd => return src_builder.pathFromCwd(self.path),
            .generated_root => |gen| {
                const gen_path = gen.path orelse {
                    std.debug.getStderrMutex().lock();
                    const stderr = std.io.getStdErr();
                    dumpBadGetPathHelp(gen.step, stderr, src_builder, asking_step) catch {};
                    @panic("misconfigured build script");
                };
                return src_builder.pathJoin(&.{ gen_path, self.path });
            },
            ...
            .generated_dirname => |gen| { // TODO(lacc97): get rid of this
                ...
                var p = getPath2(LazyPath{ .owner = self.owner, .path = self.path, .root = .{ .generated = gen.generated } }, src_builder, asking_step);
                var i: usize = 0;
                while (i <= gen.up) : (i += 1) {
                    // path is absolute.
    ...
    pub fn dupe(self: LazyPath) LazyPath {
        return .{
            ...
            .root = switch (self.root) {
                .build => .build,
                .cwd => .cwd,
                .generated_root => |gen| .{ .generated_root = gen },
